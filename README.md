...
This allows us to transform arbitrary hashes into hashed ready to be passed into ActiveModel methods:
...
This allows us to define common parameter behaviours across controllers. If a given controller has a specific requirement or custom permission, however, we can use instance-level customization:
...
params.to_hash
#=> ActiveModel::Parameters::Exceptions::MissingParamters:
#=>  required parameters not found: body
...
This will use the current controller's `current_blog` method as the `scope` in your parameters classes, detailed below.
...
Association directives allow you to convert nested hashes into `Parameters` subclasses, readying nested `ActiveModel` relations. They accept the same options as attributes.
...
UserParameters.new({ profile: { name: "John Doe", email: "john.doe@example.com" } }).to_hash!
#=> ActiveModel::Parameters::Exceptions::MissingParamters:
#=>  required parameters not found: phone
```
...
As AMP is a comprehensive solution for parameter transforming can be further transformed through several means.
